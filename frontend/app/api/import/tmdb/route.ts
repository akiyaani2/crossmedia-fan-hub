...
import { searchTMDB } from '@/lib/importers/tmdb';
...
export async function GET(req: NextRequest) {
  ...
  console.log(`TMDB Import Request: query=${query}, fandom=${fandomSlug}`);

  try {
    // Fetch items from the external API
    const itemsToInsert = await searchTMDB(query, fandomSlug);
    
    if (itemsToInsert.length === 0) {
        console.log("No TMDB results found to insert.");
        return NextResponse.json({ inserted: 0, message: "No results found from TMDB." });
    }
    ...
    console.log(`TMDB Import Success: Inserted/Updated ${insertedCount} items.`);

    // Return success response
    return NextResponse.json({ inserted: insertedCount });

  } catch (err: any) {
    console.error("[API Import TMDB Error]:", err);
    // Return error response
    return NextResponse.json(
      { error: `Failed to import from TMDB: ${err.message || 'Unknown error'}` },
      { status: 500 }
    );
  }
} 