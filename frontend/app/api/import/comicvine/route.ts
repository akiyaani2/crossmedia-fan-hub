...
import { searchComicVine } from '@/lib/importers/comicvine';
...
export async function GET(req: NextRequest) {
  ...
  console.log(`ComicVine Import Request: query=${query}, fandom=${fandomSlug}`);

  try {
    // Fetch items from the external API
    const itemsToInsert = await searchComicVine(query, fandomSlug);
    
    if (itemsToInsert.length === 0) {
        console.log("No ComicVine results found to insert.");
        return NextResponse.json({ inserted: 0, message: "No results found from ComicVine." });
    }
    ...
    console.log(`ComicVine Import Success: Inserted/Updated ${insertedCount} items.`);

    // Return success response
    return NextResponse.json({ inserted: insertedCount });

  } catch (err: any) {
    console.error("[API Import ComicVine Error]:", err);
    // Return error response
    return NextResponse.json(
      { error: `Failed to import from ComicVine: ${err.message || 'Unknown error'}` },
      { status: 500 }
    );
  }
} 